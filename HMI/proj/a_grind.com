...
//M(Mask4/$85078//);前刃磨削
	...
	DEF POS_INPUT_4=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,180,18/);坐标写入键
	...
	DEF POS_INPUT_7=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,240,18/);坐标写入键
	...
	DEF CYCLE_0=(I/*0=$85041,1=$85040//$85280,$85280,,/WR2//"/NC/_N_NC_GD2_ACX/LADAO[239]"/190,10,202/280,10,60//);内圆加工-使能
	DEF CYCLE_2=(R///$85282,$85282,,/WR4//"/NC/_N_NC_GD2_ACX/LADAO[241]"/190,30,110/280,30,60//);圆心-x
	DEF CYCLE_3=(R///$85283,$85283,,/WR4//"/NC/_N_NC_GD2_ACX/LADAO[242]"/190,50,110/280,50,60//);圆心-Y
	DEF CYCLE_1=(R///$85281,$85281,,/WR4//"/NC/_N_NC_GD2_ACX/LADAO[240]"/190,70,110/280,70,60//);圆心-z
	DEF CYCLE_4=(R///$85284,$85284,,/WR4//"/NC/_N_NC_GD2_ACX/LADAO[243]"/160,90,110/280,90,60//);磨削起点-z
	DEF CYCLE_5=(R///$85285,$85285,,/WR4//"/NC/_N_NC_GD2_ACX/LADAO[244]"/190,110,110/280,110,60//);磨削深度-y
	DEF CYCLE_6=(R///$85286,$85286,,/WR4//"/NC/_N_NC_GD2_ACX/LADAO[245]"/190,130,110/280,130,60//);磨削圈数
	DEF MSG_0=(R///$85663,$85663,,/WR4///250,150,200/0,0,0//);msg
	DEF MSG_1=(R///$85664,$85664,,/WR4///250,170,200/0,0,0//);msg
	...
	CHANGE(Z_POSITION_INIT)
		...
		IF CIJU_MODE.VAL==0
			Z_LAST_INIT.VAL=Z_POSITION_INIT.VAL+(CALC_LAST.VAL-CALC_1ST.VAL)
		ELSE
			Z_LAST_INIT.VAL=Z_POSITION_INIT.VAL+GRIND_DIERCTION.VAL*PITCH.VAL*(SLOT_AMOUNT.VAL-1)
		ENDIF
	END_CHANGE
	...
	CHANGE(Y_POSITION_INIT)
		IF DIMIANCISHENG.VAL==0
			Y_LAST_INIT.VAL=Y_POSITION_INIT.VAL
		ELSE
			Y_LAST_INIT.VAL=Y_POSITION_INIT.VAL+CISHENG_AMOUNT.VAL
		ENDIF
	END_CHANGE
...
//M(Mask6/$85080//);后背磨削
	...
	CHANGE(Z_POSITION_INIT)
		...
		IF CIJU_MODE.VAL==0
			Z_LAST_INIT.VAL=Z_POSITION_INIT.VAL+(CALC_LAST.VAL-CALC_1ST.VAL)+GRIND_DIERCTION.VAL*PITCH.VAL;齿背通过最后一齿减第二齿再加上标准齿距计算
		ELSE
			Z_LAST_INIT.VAL=Z_POSITION_INIT.VAL+GRIND_DIERCTION.VAL*PITCH.VAL*(SLOT_AMOUNT.VAL-1)
		ENDIF
	END_CHANGE
	...
	CHANGE(CIJU_MODE)
		CALC_LAST.VAR="/NC/_N_NC_GD2_ACX/MEASURE_RESULT["<<SLOT_AMOUNT.VAL+1350<<"]"
		IF CIJU_MODE.VAL==0
			Z_LAST_INIT.VAL=Z_POSITION_INIT.VAL+(CALC_LAST.VAL-CALC_1ST.VAL)+GRIND_DIERCTION.VAL*PITCH.VAL
		ELSE
			Z_LAST_INIT.VAL=Z_POSITION_INIT.VAL+GRIND_DIERCTION.VAL*PITCH.VAL*(SLOT_AMOUNT.VAL-1)
		ENDIF
	END_CHANGE
	...
	CHANGE(Y_POSITION_INIT)
		IF DIMIANCISHENG.VAL==0
			Y_LAST_INIT.VAL=Y_POSITION_INIT.VAL
		ELSE
			Y_LAST_INIT.VAL=Y_POSITION_INIT.VAL+CISHENG_AMOUNT.VAL-Y_DELTA_1.VAL
		ENDIF
	END_CHANGE
...
//M(Mask5/$85079//);顶刃磨削
	...
	DEF POS_INPUT_1=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,60,18/);坐标写入键
	...
	DEF POS_INPUT_2=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,80,18/);坐标写入键
...
//M(Mask7/$85081//);顶部侧刃
	...
	DEF POS_INPUT_3=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,140,18/);坐标写入键
	...
	CHANGE(GRIND_START)
		CALC_LAST.VAR="/NC/_N_NC_GD2_ACX/MEASURE_RESULT["<<SLOT_AMOUNT.VAL+300<<"]"
		IF CIJU_MODE.VAL==0
			Z_LAST_INIT.VAL=GRIND_START.VAL+(CALC_LAST.VAL-CALC_1ST.VAL)
		ELSE
			Z_LAST_INIT.VAL=GRIND_START.VAL+GRIND_DIERCTION.VAL*PITCH.VAL*(SLOT_AMOUNT.VAL-1)
		ENDIF
	END_CHANGE
...
//M(Mask8/$85086//);齿平面
	...
	DEF POS_INPUT_4=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,140,18/);坐标写入键
	...
	DEF POS_INPUT_7=(I/*0=$85017,1=$85017/0/$85058,$85058,,/WR2///0,0,0/530,200,18/);坐标写入键
